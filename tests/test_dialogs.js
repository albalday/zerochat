...
test('Application does not call native browser alerts', () => {
  function check(dir) {
    for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) check(file);
        ...
        assert.doesNotMatch(source, /(?:window|globalThis|self)\s*\.\s*alert\s*\(/, file);
        assert.doesNotMatch(source.replace(/function alert\(/g, 'function internalNotice('), /(?<![\w.])alert\s*\(/g, file);
      }
    }
  }
  check('js');
