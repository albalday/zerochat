...
(function (root, factory) {
  ...
  const GLYPHS = {
    ...
    'user-plus': '<path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"></path><circle cx="9" cy="7" r="4"></circle><line x1="19" y1="8" x2="19" y2="14"></line><line x1="16" y1="11" x2="22" y2="11"></line>',
    ...
    'circle-dot': '<circle cx="12" cy="12" r="10"></circle><circle cx="12" cy="12" r="4"></circle>',
  ...
  function get(name, options = {}) {
    const key = name === 'new-chat' ? 'square-pen' : name;
    const glyph = GLYPHS[key];
    if (!glyph) {
      if (typeof console !== 'undefined' && console.warn) {
        console.warn(`[ChatIcons] Icono no encontrado en catálogo: "${name}"`);
      }
    ...
    const safeTitle = options.title ? String(options.title).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;') : '';
    const titleAttr = safeTitle ? `<title>${safeTitle}</title>` : '';
